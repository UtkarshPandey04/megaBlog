...
function PostCard({$id, title, featuredImage, category, tags}) {
  return (
    <Link to={`/post/${$id}`}>
      <article className='group motion-fade-up flex h-full flex-col overflow-hidden rounded-2xl border border-slate-900/10 bg-white/80 shadow-sm transition duration-300 hover:-translate-y-1.5 hover:border-slate-900/20 hover:shadow-xl'>
        <div className='relative aspect-[4/3] w-full overflow-hidden bg-slate-100'>
          ...
          />
          <div className='pointer-events-none absolute inset-0 bg-gradient-to-t from-slate-950/30 via-transparent to-transparent opacity-70 transition group-hover:opacity-90' />
        </div>
